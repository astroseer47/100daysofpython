...
def search_password():
    ...
    with open("./passwords.json", "r") as file:
        data = json.load(file)
        try:
            if data[website]['username'] != "":
                messagebox.showinfo("Password Details", f"username: {data[website]['username']} | password: {data[website]['password']}")
        except KeyError:
            messagebox.showerror("NotFound", "Could not find details")
...
website_url = Entry(name="website_url" , width=18)
website_url.grid(row=1,column=1)

website_search_button = Button(text="Search", width=18, command=search_password)
...
password_entry = Entry(name="password", width=20)
